...
if [ $REANALYSIS = ERA5 ] 
  ...
  while [ $TARGETYEAR -le $TARGETEND ]
    do 
    if [ $SOURCEYEAR -eq 1996 ]; then
      ln -fs ERAI_lwdown_$SOURCEYEAR ERA_lwdown_$TARGETYEAR
      ln -fs ERAI_swdown_$SOURCEYEAR ERA_swdown_$TARGETYEAR
      ln -fs ERAI_apressure_$SOURCEYEAR ERA_apressure_$TARGETYEAR
      ln -fs ERAI_precip_$SOURCEYEAR ERA_precip_$TARGETYEAR
      ln -fs ERAI_aqh_$SOURCEYEAR ERA_aqh_$TARGETYEAR
      ln -fs ERAI_atemp_$SOURCEYEAR ERA_atemp_$TARGETYEAR
      ln -fs ERAI_uwind_$SOURCEYEAR ERA_uwind_$TARGETYEAR
      ln -fs ERAI_vwind_$SOURCEYEAR ERA_vwind_$TARGETYEAR
    else
      ln -fs ERA5_lwdown_$SOURCEYEAR ERA_lwdown_$TARGETYEAR
      ln -fs ERA5_swdown_$SOURCEYEAR ERA_swdown_$TARGETYEAR
      ln -fs ERA5_apressure_$SOURCEYEAR ERA_apressure_$TARGETYEAR
      ln -fs ERA5_precip_$SOURCEYEAR ERA_precip_$TARGETYEAR
      ln -fs ERA5_aqh_$SOURCEYEAR ERA_aqh_$TARGETYEAR
      ln -fs ERA5_atemp_$SOURCEYEAR ERA_atemp_$TARGETYEAR
      ln -fs ERA5_uwind_$SOURCEYEAR ERA_uwind_$TARGETYEAR
      ln -fs ERA5_vwind_$SOURCEYEAR ERA_vwind_$TARGETYEAR
    fi
  ...
  while [ $TARGETYEAR -le $SIMENDYEAR ]
    do
    if [ $TARGETYEAR -eq 1996 ]; then
      ln -fs ERAI_uwind_$TARGETYEAR ERA_uwind_$TARGETYEAR
      ln -fs ERAI_vwind_$TARGETYEAR ERA_vwind_$TARGETYEAR
      ln -fs ERAI_lwdown_$TARGETYEAR ERA_lwdown_$TARGETYEAR
      ln -fs ERAI_swdown_$TARGETYEAR ERA_swdown_$TARGETYEAR
      ln -fs ERAI_apressure_$TARGETYEAR ERA_apressure_$TARGETYEAR
      ln -fs ERAI_precip_$TARGETYEAR ERA_precip_$TARGETYEAR
      ln -fs ERAI_aqh_$TARGETYEAR ERA_aqh_$TARGETYEAR
      ln -fs ERAI_atemp_$TARGETYEAR ERA_atemp_$TARGETYEAR
      ln -fs ERAI_uwind_$TARGETYEAR ERA_uwind_$TARGETYEAR
    else
      ln -fs ERA5_uwind_$TARGETYEAR ERA_uwind_$TARGETYEAR
      ln -fs ERA5_vwind_$TARGETYEAR ERA_vwind_$TARGETYEAR
      ln -fs ERA5_lwdown_$TARGETYEAR ERA_lwdown_$TARGETYEAR
      ln -fs ERA5_swdown_$TARGETYEAR ERA_swdown_$TARGETYEAR
      ln -fs ERA5_apressure_$TARGETYEAR ERA_apressure_$TARGETYEAR
      ln -fs ERA5_precip_$TARGETYEAR ERA_precip_$TARGETYEAR
      ln -fs ERA5_aqh_$TARGETYEAR ERA_aqh_$TARGETYEAR
      ln -fs ERA5_atemp_$TARGETYEAR ERA_atemp_$TARGETYEAR
      ln -fs ERA5_uwind_$TARGETYEAR ERA_uwind_$TARGETYEAR
    fi
    TARGETYEAR=$((TARGETYEAR+1))
  done
