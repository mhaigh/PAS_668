...
# Subtract 3 minutes
RUNTIME="$(($rem_secs-300))"
echo Will run for $RUNTIME sec >> walltime

# Run the job but leave 5 minutes at the end
...
# Here get slurm/job id. Create dir on BSL with this id.
# Copy all state.data files. Delete all state files. 
# Restart new run.
